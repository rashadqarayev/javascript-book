...
Bu kitabda dəfələrlə qarşılaşdığımız **iterable (iterasiya edilə bilən)** obyektlər və onların **iteratorları** ES6-nın vacib bir xüsusiyyətidir. Massivlər (Arrays), Stringlər, `Set` və `Map` obyektləri iterable-dır. Bu o deməkdir ki, bu data strukturlarının daxilindəki elementlər üzərində dövr etmək (loop) mümkündür.

Bunu ən çox **`for...of`** dövrü ilə görmüşük:
```javascript
let sum = 0;
for (let i of [1, 2, 3]) { 
  // Bu dəyərlərin hər biri üçün bir dəfə dövr edir
  sum += i;
}
console.log(sum); //  6
```
...
`Map` obyektləri üzərində iterasiya apardıqda isə, hər element `[açar, dəyər]` cütlüyü şəklində qayıdır ki, bu da `for...of` dövründə görünür.
```javascript
...
console.log(...myMap.keys());   //  bir iki
console.log(...myMap.values()); //  1 2
```

---
### 12.1 Iteratorlar Necə İşləyir?
...
JavaScript-in daxili (built-in) iteratorları özləri də iterable-dır. Yəni, bir iteratorun özünün də `[Symbol.iterator]` metodu var və bu metod sadəcə özünü qaytarır.
...
console.log("İlk rəqəm:", ilkReqem); // 10
...
console.log("Qalan rəqəmlər:", qalanReqemler); //  [20, 30, 40, 50]
```

***
### 12.2 Iterable Obyektlərin Yaradılması (Implementing Iterable Objects)
...
console.log("Aralıqdan yaranan massiv (array):", arr); 
//  [-2, -1, 0, 1, 2]
```
...
// "Generator başladı..." -> { value: 2, done: false }
console.log(primes.next()); 
// "İkinci dəyərə keçdi..." -> { value: 3, done: false }
console.log(primes.next()); 
// "Üçüncü dəyərə keçdi..." -> { value: 5, done: false }
console.log(primes.next()); 
// "Dördüncü dəyərə keçdi..." -> { value: 7, done: false }
console.log(primes.next()); 
// "Generator bitdi." -> { value: undefined, done: true }
...
```
 ...
 Unutma: Arrow funksiyası (`=>`) sintaksisi ilə generator yaratmaq mümkün deyil.
...
console.log("20-ci Fibonacci rəqəmi:", fibonacci(20)); // 6765
```
...
Təsəvvür et ki, bir neçə fərqli iterable obyekti (məsələn, massiv (array),  string, generator) birləşdirib, onların elementlərini bir-birinin ardınca, ardıcıl şəkildə qaytaran bir generator yazmaq istəyirik. Bunu iç-içə `for...of` dövrü ilə belə yaza bilərik:
...
const numbers = [1, 2];
const letters = "ab";
console.log([...sequence(numbers, letters)]); 
// [1, 2, "a", "b"]
```
...
const numbers = [1, 2];
const letters = "ab";
console.log([...sequenceWithYieldStar(numbers, letters)]); // [1, 2, "a", "b"]
```
...
// ["Elektronika", "Telefonlar", "Kompüterlər", "Notbuklar", "Masaüstü"]
```
...
console.log("Normal iterasiya:", normalIteration); // [1, 2]
...
function* twoWayCommunication() {
  ...
  let firstInput = yield "İlk sual: Adın nədir?"; 
  // 1. Dayandı, "İlk sual..." göndərdi
  console.log(`📥 Generatora gələn ilk cavab: ${firstInput}`);
  
  let secondInput = yield "İkinci sual: Yaşın neçədir?"; 
  // 2. Dayandı, "İkinci sual..." göndərdi
...
/*
Generator başladı.
VM113:19 📤 Generatordan gələn ilk sual: İlk sual: Adın nədir?
VM113:6 📥 Generatora gələn ilk cavab: Elvin
VM113:24 📤 Generatordan gələn ikinci sual: İkinci sual: Yaşın neçədir?
VM113:10 📥 Generatora gələn ikinci cavab: 28
VM113:28 📤 Generatordan gələn son nəticə: Dialoq bitdi.
*/
```
---
...
// { value: 1, done: false }
// 🧼 Təmizlik işləri aparılır (finally bloku).
// Dövrü yarımçıq dayandırırıq
console.log(gen1.return("Bitdi")); 
// {value: "Bitdi", done: true}
...
// ❗️ Generator daxilində xəta tutuldu: Xarici müdaxilə
// 🧼 Təmizlik işləri aparılır (finally bloku).
// { value: undefined, done: true }
```
---
